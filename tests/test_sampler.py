...
import time
import conftest
from liveprofiler_sampler import ProfilingMiddleware

def test_middleware_happy_path(simple_app):
    '''
    ...
    response = json.loads(simple_app.get('/liveprofiler', headers=headers).text)
    assert 'elapsed' in response.keys()
    assert response['granularity'] == 1

def test_middleware_wrong_secret(simple_app):
    '''
    required: method GET, good PROFILER_TOKEN, request_path doesnt matter
    '''
    headers = {ProfilingMiddleware.SECRET_HEADER_NAME: 'wrong_secret'}
    response = simple_app.get('/liveprofiler', headers=headers, status=403)
    assert response.status_code == 403

def test_middleware_wrong_secret_header_key(simple_app):
    '''
    required: method GET, good PROFILER_TOKEN, request_path doesnt matter
    '''
...
def test_middleware_non_profiling_call(simple_app):
    assert simple_app.get('/').text == 'app_endpoint'
    assert simple_app.post('/post_test').text == 'app_endpoint'
